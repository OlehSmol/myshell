...
int main(int argc, char *argv[]) {
    ...
    if(opt.files.size() == 0){
        std::cout<<"mv: missing file operand\n";
        return 0;
        ...
        if(stat(opt.files[0].c_str(), &st_in) == -1){
            std::cout<< "mv: cannot move '" + opt.files[0] + "': No such file or directory\n";
            return 0;
        }
        ...
        if(S_ISDIR(st_in.st_mode)){
            if(opt.recursive == true){
                ...
                std::cout<< "mv: omitting directory '/" + opt.files[0] + "'\n";
            }
        } else {
            int fd_w;
            struct stat st_out;
            if(stat(opt.files[1].c_str(), &st_out) == -1){
                ...
                if(fd_w == -1){
                    std::cout<< "mv: cannot move to '" + opt.files[1] + "': No such file or directory\n";
                    return -1;
                }
            ...
            if(fd_r == -1){
                std::cout<< "mv: can`t open file for reading\n";
                return -1;
            }
            ...
            close(fd_r);
            close(fd_w);
            if(remove(opt.files[0].c_str()) == -1){
                std::cout<< "mv: cannot remove '/" + opt.files[0] + "'\n";
            }
        }
    } else {
        struct stat st_out;
        if(stat(opt.files[opt.files.size()-1].c_str(), &st_out) == -1){
            std::cout<<"mv: cannot move to '" + opt.files[opt.files.size()-1] + "': No such file or directory\n";
            return -1;
        } else if(!S_ISDIR(st_out.st_mode)){
            std::cout<<"mv: target '" + opt.files[opt.files.size()-1] + "' is not a directory\n";
            return -1;
        } else{
            for(int i=0; i<opt.files.size()-1; i++){
                struct stat st_in;
                if(stat(opt.files[i].c_str(), &st_in) == -1){
                    std::cout<< "mv: cannot move '" + opt.files[i] + "': No such file or directory\n";
                    return 0;
                }
                if(S_ISDIR(st_in.st_mode)){
                    if(opt.recursive == true){
                        ...
                        std::cout<< "mv: omitting directory '/" + opt.files[i] + "'\n";
                    }
                } else {
                    ...
                    close(fd_r);
                    close(fd_w);
                    if(remove(opt.files[i].c_str()) == -1){
                        std::cout<< "mv: cannot remove '/" + opt.files[i] + "'\n";
                    }
                }
            }
        }
